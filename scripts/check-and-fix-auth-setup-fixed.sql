-- Comprehensive Auth & Database Setup Check (FIXED)
...
BEGIN
    ...
    -- 7. Check RLS policies (simplified to avoid complex queries)
    RAISE NOTICE '';
    RAISE NOTICE '🛡️ ROW LEVEL SECURITY:';
    RAISE NOTICE '  Checking if RLS is enabled on key tables...';
    ...
    ELSE
        RAISE NOTICE '  ❌ RLS not enabled on users table';
    END IF;
